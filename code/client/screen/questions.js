...
export default function (game) {
	...
	function onNewQuestion(msg) { // quand on reçoit une nouvelle question
		...
		majAffichage(msg); // on passe à l'état "en attente de la bonne réponse"
	}

	game.onStart = function (msg) { // quand la partie commence

		setup(); // on affiche l'écran de la question
	...
	game.onMessage = function (msg) {
		...
		if (msg[0] == NEW_SCREEN) {
			...
			for (let i = 0; i < 4; i++) {
				questionAnswers[i] = msg[2][2 + i]; // on ajoute la réponse reçue à la listes des réponses possibles
			...
			if (msg[1] == 3) {
				...
				questionDiv.innerHTML = "La prochaine question va bientôt arriver !";
				let answersText = document.getElementsByClassName("answer-text");
				for (let i = 0; i < 4; i++) {
					//					answers[i].style.backgroundColor = ""; // on réinitialise la couleur des encadrés de réponse
					answersText[i].innerHTML = "";
				}
	...
	function majAffichage(msg) {
		...
		if (questionNumber) {
			...
			if (msg[7]) {
				txt = msg[7];
				questionNumber = msg[7];
			} else {
				txt = "...";
			}
		}
		questionNumberDiv.innerHTML = "Question " + txt; // on affiche le numéro de la nouvelle question
